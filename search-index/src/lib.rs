...
impl ItemIndex {
    ...
    pub fn query_top(&self, term: &str, limit: usize) -> tantivy::Result<Vec<ItemDoc>> {
        ...
        let parser = QueryParser::for_index(&self.index, vec![name_field, desc_field]);
        let query = parser.parse_query(term)?;

        let searcher = self.reader.searcher();
        let mut docs = searcher.search(&query, &collector)?;

        if docs.is_empty() {
            let term = Term::from_field_text(name_field, term);
            let query = FuzzyTermQuery::new(term, 1, true);
            docs = searcher.search(&query, &collector)?;
        }
        ...
        for (_, addr) in docs.into_iter() {
            ...
            let item = ItemDoc {
                ...
                name: names[0].text().unwrap_or_default().to_string(),
                short_name: names[1].text().unwrap_or_default().to_string(),
                description: doc
